...
def test_live_stream_defaults_to_twelve_turns(app_client) -> None:
    response = app_client.get(
    ...
    assert response.status_code == 200
    events = _parse_sse(response.get_data(as_text=True))
    assert [event["event"] for event in events].count("turn") == 12
    assert [event["event"] for event in events].count("referee") == 12
    assert events[-1]["data"]["meta"]["total_turns"] == 12
    assert events[-1]["data"]["meta"]["fact_referee"]["judged_turns"] == 12
    assert len(events[-1]["data"]["turns"]) == 12
